...
export default function PdfViewer({ pdfUrl }: PdfViewerProps) {
    ...
    useEffect(() => {
        ...
        function updateWidth() {
            ...
            setContainerWidth(
                Math.max(280, Math.min(container.clientWidth - 24, 1000)),
            );
        }
    ...
    function zoomIn() {
        setZoom((currentZoom) =>
            Math.min(Number((currentZoom + 0.15).toFixed(2)), 2.5),
        );
    }

    function zoomOut() {
        setZoom((currentZoom) =>
            Math.max(Number((currentZoom - 0.15).toFixed(2)), 0.5),
        );
    }
